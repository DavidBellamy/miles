...
When using independent data parallelism (`indep_dp`), miles automatically monitors trainer actors via heartbeat checks. A background async task periodically calls each actor's `heartbeat()` method and compares the returned last-active timestamp against the current time. If the timestamp is stale (exceeding `--trainer-heartbeat-checker-staleness`) or the RPC times out, the cell is marked as errored and excluded from subsequent training rounds.
...
- `--trainer-heartbeat-checker-first-wait`: Initial grace period before starting heartbeat checks, allowing time for model initialization and compilation. Defaults to 300s.
- `--trainer-heartbeat-checker-interval`: The interval between heartbeat checks. Defaults to 30s.
- `--trainer-heartbeat-checker-timeout`: The timeout for a single heartbeat RPC. Defaults to 10s.
- `--trainer-heartbeat-checker-staleness`: Maximum allowed staleness of the last-active timestamp. Defaults to 90s.
